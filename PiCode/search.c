...
int totalDepth(search_t *search, int depthPhase1, int maxDepth) {
    ...
    if ((d1 = getPruning(Slice_URFtoDLF_Parity_Prun,
                         ...
                         search->parity[depthPhase1])) > maxDepthPhase2)
        return -1;
    ...
    do {
        do {
            ...
            } else if ((search->ax[n] == 0 || search->ax[n] == 3) ? (++search->po[n] > 3) : (
                ...
                do {// increment axis
                    if (++search->ax[n] > 5) {
                        ...
                        busy = 0;
                         ...
                         (search->ax[n - 1] == search->ax[n] || search->ax[n - 1] - 3 == search->ax[n]));
            } else
                busy = 0;
        } while (busy);
        ...
        search->minDistPhase2[n + 1] = MAX(getPruning(Slice_URtoDF_Parity_Prun, (N_SLICE2
                                                                                 * search->URtoDF[n + 1] +
                                                                                 search->FRtoBR[n + 1])
                                                                                * 2 + search->parity[n + 1]),
                                           getPruning(Slice_URFtoDLF_Parity_Prun, (N_SLICE2
                                                                                   * search->URFtoDLF[n + 1] +
                                                                                   search->FRtoBR[n + 1])
                                                                                  * 2 + search->parity[n + 1]));
        // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
